...
// import { View, Alert, Vibration, Image, LogBox, BackHandler, Modal, Linking, TouchableOpacity, FlatList, Text, TextInput, SectionList, ScrollView, KeyboardAvoidingView, TouchableWithoutFeedback } from 'react-native';
...
//                         <View style={{ height: 50, flexDirection: 'row', alignItems: 'center', borderRadius: 10, width: '60%', backgroundColor: COLOR.white, shadowOpacity: 0.2, shadowRadius: 5, shadowOffset: { height: 2, width: 2 }, alignSelf: 'center', position: 'absolute', top: -30, justifyContent: 'center' }}>
...
//                             <TouchableOpacity style={{ flexDirection: 'row', padding: 10 }} onPress={() => { setReMeCkModal(true), setMsgType('Task') }}>
...
//                                     {<CtrateHeader source={msgType == 'Task' ? '' : userDetails.profile} onBack={() => setReMeCkModal(false)} title={msgType == 'Task' ? 'Create Task List' : userName?.length >= 20 ? userName?.slice(0, 15) + ' . . . ' : userName} />}
                    ...
                    : <View style={{ flex: 1 }}>
                        {msgType == 'Contact' ?
                            ...
                            <View style={styles.chatMainsection}>
                                ...
                                <Modal visible={ReMeCkModal} animationType='fade'>
                                    <View style={styles.createItemModalView}>
                                        {<CtrateHeader source={msgType == 'Task' ? '' : userDetails?.profile} onBack={() => { setReMeCkModal(false), setSelectedMSG([]) }} title={msgType == 'Task' ? 'Create Task List' : userName?.length >= 20 ? userName?.slice(0, 15) + ' . . . ' : userName} />}
